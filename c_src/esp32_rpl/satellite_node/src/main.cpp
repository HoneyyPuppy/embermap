...
void loop() {
    ...
    if (currentMillis - lastSensorReadTime >= SENSOR_READ_INTERVAL) {
        ...
        SensorService::updateAlarm(isEmergency, routingTable.hasParent());
        
        uint8_t parent[6];
        memcpy(parent, routingTable.getParentMac(), 6);
        SensorService::displaySatellite("RPL", SATELLITE_ID, currentTemp, currentGas, routingTable.hasParent(), routingTable.getRank(), parent);

        meshNetwork.sendSensorData(currentTemp, currentGas, isEmergency);
    }

    if (routingTable.hasParent() && (currentMillis - lastDioRebroadcastTime >= DIO_REBROADCAST_INTERVAL)) {
        lastDioRebroadcastTime = currentMillis;
        meshNetwork.broadcastDio();
    }

    if (routingTable.hasParent() && (currentMillis - lastDaoSendTime >= DAO_SEND_INTERVAL)) {
