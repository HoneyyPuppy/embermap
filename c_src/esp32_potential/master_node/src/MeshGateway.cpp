...
bool MeshGateway::init() {
    ...
    Serial.printf("\n=========================================\n");
    Serial.printf("MAC MASTER (APF): %02X:%02X:%02X:%02X:%02X:%02X\n", 
...
void MeshGateway::broadcastPotential() {
    MeshPacket packet = {};
    ...
    // Đối với APF Master, thế năng nguồn hấp dẫn (Destination/Master) luôn là 0.0, hopCount = 0
    packet.potential = 0.0;
    packet.hopCount = 0;
    ...
    Serial.printf("[APF Master] Broadcast Potential (Potential = 0.0) -> %s\n", result == ESP_OK ? "SUCCESS" : "FAIL");
}
...
void MeshGateway::handleRecv(const esp_now_recv_info_t *recv_info, const MeshPacket& packet) {
    if (packet.packetType == PACKET_SENSOR_DATA) {
        if (memcmp(packet.forwardMac, m_myMac, 6) == 0) {
            Serial.printf("[APF] Received sensor from Node %d. Temp: %.1f C, Gas: %d\n", 
