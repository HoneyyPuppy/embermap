...
typedef struct __attribute__((packed)) {
  ...
  uint8_t forwardMac[6];    // MAC address of next hop (Parent)
  int id;                   // ID of origin node
  float temp;               // Temperature data
  int gasRaw;               // MQ-2 gas data
...
const unsigned long DIO_BROADCAST_INTERVAL = 5000;  // Quảng bá DIO mỗi 5 giây
...
// ==================== HÀM PHÁT QUẢNG BÁ DIO (DODAG INFORMATION OBJECT) ====================
void broadcastDio() {
  MeshPacket packet;
  ...
  packet.id = 0;             // ID = 0 là Root Node
  packet.temp = 0.0;
  packet.gasRaw = 0;
  packet.emergency = false;
  packet.rank = 256;         // Gốc DODAG có Rank = 256 (Root Rank)
  packet.version = 1;        // Phiên bản DODAG = 1
...
void OnDataRecv(const esp_now_recv_info_t *recv_info, const uint8_t *incomingDataRaw, int len) {
  ...
  // 1. Nhận gói DAO (Destination Advertisement Object) báo cáo lộ trình ngược
...
void setup() {
  ...
  // Đăng ký Peer quảng bá để có thể gửi gói tin định tuyến DIO
  ...
  // Phát DIO ngay lập tức lúc khởi động để xây dựng cây DODAG
  broadcastDio();
}
...
void loop() {
  ...
  // 1. Định kỳ phát quảng bá DIO (xây dựng cây DODAG cho RPL)
